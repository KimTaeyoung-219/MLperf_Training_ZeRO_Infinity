...
import logging
class TFRecordLLMGenerator(DataGenerator):
    """
    ...
    def generate(self):
        ...
        record_label = 0
        dim = self._dimension
        logging.info(f"file number: {len(self._file_list)}, data per file: {self.num_samples}, total samples: {len(self._file_list * self.num_samples)}")
        for i in dlp.iter(range(self.my_rank, self.total_files_to_generate, self.comm_size)):
            progress(i+1, self.total_files_to_generate, "Generating TFRecord LLM Data")
            out_path_spec = self.storage.get_uri(self._file_list[i])
            # dim1 = dim[2*i]
            # dim2 = dim[2*i+1]
            # logging.info(f"(dim1, dim2): {dim1}, {dim2}")
            # Open a TFRecordWriter for the output-file.
            with tf.io.TFRecordWriter(out_path_spec) as writer:
                for i in range(0, self.num_samples):
                    # This creates a 2D image representing a single record
                    record = self.generate_random_string(dim)
                    # logging.info(f"{type(record)}")
                    img_bytes = record.tobytes()
                    data = {
                        'text': tf.train.Feature(bytes_list=tf.train.BytesList(value=[img_bytes])),
