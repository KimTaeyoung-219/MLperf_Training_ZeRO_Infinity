...
class DLIOBenchmark(object):
    ...
    def _train(self, epoch):
        ...
        if self.ZeRO_Infinity is True:
            self.stats.start_block_ZeRO_Infinity(epoch, block, self.args)
        ...
        for batch in dlp.iter(loader.next()):
            ...
            if self.computation_time > 0:
                ...
                if self.ZeRO_Infinity == False:
                    # logging.info(f"No ZeRO_Infinity")
                    self.framework.compute(batch, epoch, block_step, computation_time)
                else:
                    # logging.info(f"ZeRO_Infinity")
                    computation_time = self.framework.compute_ZeRO_Infinity(batch, epoch, block_step, self.args, self.stats, block)
            self.comm.barrier()
    ...
    def run(self):
        ...
        if not self.generate_only:
            ...
            for epoch in range(1, self.epochs + 1):
                ...
                if self.ZeRO_Infinity is True:
                    self.stats.start_ZeRO_Infinity(epoch, self.args.parameter_swapping, self.args.gradient_swapping, self.args.optimizer_swapping)
                steps = self._train(epoch)
                self.stats.end_train(epoch, steps)
                if self.ZeRO_Infinity is True:
                    self.stats.end_train_ZeRO_Infinity(epoch, steps)
        ...
        self.stats.end_run()
        if self.ZeRO_Infinity is True:
            self.stats.end_run_ZeRO_Infinity()
